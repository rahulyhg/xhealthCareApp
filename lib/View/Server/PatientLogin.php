...
class View_Server_PatientLogin extends \View{
	function init(){
		parent::init();
		
		$this->add('H1')->set('Patient Login');
		$form=$this->add('Form');
		...
		if($form->isSubmitted()){
			...
			$doctor=$this->add('healthCareApp/Model_Patient');
		 	if(!$doctor->tryLogin($form['email'],$form['password']))
				...
				$this->js()->univ()->redirect($this->api->url(null,array('subpage'=>'xhealthcare-ptdashboard')))->execute();
			}
	}
}