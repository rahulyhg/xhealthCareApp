...
class View_Server_NurseLogin extends \View{
	function init(){
		parent::init();
		
		$this->add('H1')->set('Nurse Login');
		$form=$this->add('Form');
		...
		if($form->isSubmitted()){
			...
			$doctor=$this->add('healthCareApp/Model_Nurse');
		 	if(!$doctor->tryLogin($form['email'],$form['password']))
		 		$form->displayError('email','wrong credentials');
		 	
				// Redirect to Dashboard
				$this->js()->univ()->redirect($this->api->url(null,array('subpage'=>'xhealthcare-nsdashboard')))->execute();
			}
	}
}